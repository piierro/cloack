...
import AppTableCampaigns from "@/components/controls/tables/app-table-campaigns";
import CampaignItem from "@/components/common/renders/campaign-item";
import { useRouter } from "next/navigation";
import styles from './styles/lk-user.module.css';
...
export default function LkUserContainer() {
    ...
    return (
        ...
        <Toaster />
        <NavBar data={routes} />
        <div className="unauthorized-error error-field mx-5">{!isLoading && !auth.authorized ? "Your account subscribe is expired! Renew your subscription to do any actions on account." : ''}</div>
        <div className={`${styles.content} mt-10`}>
            <div className={`${styles.label} title-page mb-10`}>
                <div className="text-3xl bg-card rounded-lg pt-6 pb-6 pl-5">Campaigns</div>
            </div>
            <div className={` ${styles.searchContent} wrapper-search flex align-end justify-end mb-8`}>
            <div className={`${styles.searchCont} search flex items-center justify-start`}>
                <InputIconComponent 
                   icon={<Search />}
                   type="string" 
